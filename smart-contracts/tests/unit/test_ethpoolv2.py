...
from scripts.deploy import deploy_ethpool_v2
import pytest
...
def _init():
    ...
    return get_account(), deploy_ethpool_v2()
...
def test_can_withdraw_exact():
    ...
    # Assert
    assert initBalanceOwner == stakingToken.balanceOf(owner.address)
    assert initBalanceUserA == stakingToken.balanceOf(owner.address)
...
def test_no_rewards_to_claim_if_user_didnt_deposit():
    ...
    _fund_accounts([owner, userA, userB], "fau", toStakeAmount)
    _approve_accounts(
        [owner, userA, userB], "fau", ethpool.address, Web3.toWei(100, "ether")
    )
    ...
    for _ in range(0, periods):
        ethpool.depositRewards(toRewardAmount, {"from": owner}).wait(1)

    # Assert
    with pytest.raises(exceptions.VirtualMachineError):
        ethpool.claimRewards({"from": userA}).wait(1)

    assert ethpool.getRewardBalance() == toRewardAmount * periods
...
def test_can_deposit_rewards():
    ...
    # Assert
    ethpool.getRewardBalance() == stakingToken.balanceOf(ethpool.address)
    ethpool.getRewardBalance() == toRewardAmount

    # Only owner
    with pytest.raises(exceptions.VirtualMachineError):
        ethpool.depositRewards(toRewardAmount, {"from": userA}).wait(1)
...
def test_can_claim_rewards_after_withdraw():
    ...
    # t1
    ethpool.depositRewards(owner_toReward, {"from": owner}).wait(1)
...
def test_can_deposit_multiple_times_and_claim():
    ...
    # t1
    ethpool.depositRewards(Web3.toWei(1, "ether"), {"from": owner}).wait(1)
    ...
    # t3
    ethpool.depositRewards(Web3.toWei(3, "ether"), {"from": owner}).wait(1)
...
def test_custom_scenario():
    ...
    # t1
    ethpool.depositRewards(Web3.toWei(10, "ether"), {"from": owner}).wait(1)
    ...
    # t2
    ethpool.depositRewards(Web3.toWei(20, "ether"), {"from": owner}).wait(1)

    ethpool.withdrawExact(Web3.toWei(30, "ether"), {"from": userC}).wait(1)

    # t3
    ethpool.depositRewards(Web3.toWei(45, "ether"), {"from": owner}).wait(1)
