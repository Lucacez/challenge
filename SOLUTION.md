...
-   Rewards are not sent to the user until they claim them. This means that the rewards can't just be paid out whenever the deposit happens, but instead the solution needs to keep track of the corresponding rewards each user is entitled to, and send them only when requested. This is what's considered a **Pull-Based Reward Distribution**.

The way I've chosen to solve this problem consists of the use of what I will denominate as **_Masks_**. These are used to represent the accumulated rewards generated by the contract up to a certain _period_ of time. Only two kinds of masks are stored:

-   `totalRewardPerTokenMask`: Only one value. Global.
-   `userRewardPerTokenMask`: Multiple values. One per User.
...
>
> In a more realistic situation, a _Period_ would be some other kind of unit, like Blocks or Time.
...
<h3 align="center"> <i>And this is how reward distribution can be done in an efficient way, avoiding loops and thus reducing gas consumption.</i> 🔥</h3>
